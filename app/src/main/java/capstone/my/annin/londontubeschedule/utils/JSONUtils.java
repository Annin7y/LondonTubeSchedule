...
public class JSONUtils {
    ...
    private static final String KEY_OVERGROUND_ALL_NAME = "commonName";
...
//    }
    ...
    public static ArrayList<OvergroundStatus> extractFeatureFromOvergroundStatusJson(String overgroundStatusJSON) {
        ...
        try {
            ...
            // For each line in the overgroundArray, create an {@link Lines} object
    ...
    public static ArrayList<OvergroundStation> extractFeatureFromOvergroundStationAllJson(String overgroundStatAllJSON) {
        // If the JSON string is empty or null, then return early.
        if (TextUtils.isEmpty(overgroundStatAllJSON)) {
            return null;
        }

        ArrayList<OvergroundStation> overgroundStations = new ArrayList<>();
        try {
            // Create a JSONObject from the JSON response string
            JSONArray overgroundArray = new JSONArray(overgroundStatAllJSON);

            // For each line in the overgroundArray, create an {@link Lines} object
            for (int i = 0; i < overgroundArray.length(); i++) {
                // Get a single line description at position i within the list of stations
                JSONObject currentOverground = overgroundArray.getJSONObject(i);
                ...
                String name = "";
                if (currentOverground.has("commonName")) {
                    name = currentOverground.optString(KEY_OVERGROUND_ALL_NAME);
                }

                double stationLatLocation = currentOverground.getDouble("lat");;
                if (currentOverground.has("lat")) {
                    stationLatLocation = currentOverground.optDouble("lat");
                }

                double stationLonLocation = currentOverground.getDouble("lon");

                if (currentOverground.has("lon")) {
                    stationLonLocation = currentOverground.optDouble("lon");
                }

                OvergroundStation station = new OvergroundStation(id, name, stationLatLocation, stationLonLocation);
            ...
            //
            //Log.e("QueryUtils", "Problem parsing lines JSON results", e);
            Timber.e("Problem parsing overground all stations JSON results");
